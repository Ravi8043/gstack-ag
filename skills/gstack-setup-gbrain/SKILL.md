...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-detect`
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-install
```
...
```bash
. ~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.gemini/config/plugins/gstack-ag/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.gemini/config/plugins/gstack-ag/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-config set transcript_ingest_mode <choice>
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-gbrain-detect 2>/dev/null || true
~/.gemini/config/plugins/gstack-ag/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.gemini/config/plugins/gstack-ag/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
