...
[ -z "$_AUTO" ] && _AUTO=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.gemini/config/plugins/gstack-ag/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/gstack-ag-dist/skills/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/gstack-ag-dist/skills"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d "gstack-ag-dist/skills/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR="gstack-ag-dist/skills"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d "gstack-ag-dist/skills" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR="gstack-ag-dist/skills"
elif [ -d "$HOME/gstack-ag-dist/skills" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/gstack-ag-dist/skills" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/gstack-ag-dist/skills" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/gstack-ag-dist/skills"
  fi
fi
_TEAM_MODE=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached gstack-ag-dist/skills/ 2>/dev/null || true
if ! grep -qF 'gstack-ag-dist/skills/' .gitignore 2>/dev/null; then
  echo 'gstack-ag-dist/skills/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `gstack-ag-dist/skills/` when you're ready."
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-update-check --force 2>/dev/null || \
gstack-ag-dist/skills/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `gstack-ag-dist/skills/` when you're ready."
