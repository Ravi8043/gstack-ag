...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat ~/.gemini/config/plugins/gstack-ag/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.gemini/config/plugins/gstack-ag/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
