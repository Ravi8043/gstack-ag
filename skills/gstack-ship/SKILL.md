...
GSTACK_ROOT="$HOME/.gemini/config/plugins/gstack-ag"
[ -n "$_ROOT" ] && [ -d "$_ROOT/gstack-ag-dist/skills" ] && GSTACK_ROOT="$_ROOT/gstack-ag-dist/skills"
GSTACK_BIN="$GSTACK_ROOT/bin"
GSTACK_BROWSE="$GSTACK_ROOT/browse/dist"
GSTACK_DESIGN="$GSTACK_ROOT/design/dist"
_UPD=$($GSTACK_BIN/gstack-update-check 2>/dev/null || gstack-ag-dist/skills/bin/gstack-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d "gstack-ag-dist/skills" ] && [ ! -L "gstack-ag-dist/skills" ]; then
  if [ -f "gstack-ag-dist/skills/VERSION" ] || [ -d "gstack-ag-dist/skills/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> This project has gstack vendored in `gstack-ag-dist/skills/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r gstack-ag-dist/skills/`
2. Run `echo 'gstack-ag-dist/skills/' >> .gitignore`
3. Run `$GSTACK_BIN/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ~/.gemini/config/plugins/gstack-ag && ./setup --team`"
...
~/.gemini/config/plugins/gstack-ag/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -x ~/.gemini/config/plugins/gstack-ag/bin/gstack-telemetry-log ]; then
  ~/.gemini/config/plugins/gstack-ag/bin/gstack-telemetry-log \
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-review-read
```
...
For Design Review: run `source <(~/.gemini/config/plugins/gstack-ag/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `gstack-ag-dist/skills/review/TODOS-format.md`).
...
```bash
eval "$(~/.gemini/config/plugins/gstack-ag/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
1. Read `gstack-ag-dist/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `gstack-ag-dist/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `gstack-ag-dist/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.gemini/config/plugins/gstack-ag/bin/gstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
Read `gstack-ag-dist/skills/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/gstack-ag-dist/skills/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
2. Compute the corrected title: `NEW_TITLE=$(~/.gemini/config/plugins/gstack-ag/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
